...
@NamedQueries({
	...
	@NamedQuery(name="Solicitud.findAudiencias", query="SELECT DISTINCT s FROM Solicitud s, Agenda ag, Audiencia au WHERE ag.fecha BETWEEN :fechaInicial AND :fechaFinal AND ag.audiencia.idAudiencia = au.idAudiencia AND au.solicitud.idSolicitud = s.idSolicitud AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
	...
	@NamedQuery(name="Solicitud.findAudienciasConciliador", query="SELECT DISTINCT s FROM Solicitud s, Designacion d, Audiencia aud, Agenda ag WHERE s.idSolicitud=d.solicitud.idSolicitud AND s.idSolicitud=aud.solicitud.idSolicitud AND aud.idAudiencia = ag.audiencia.idAudiencia AND ag.fecha BETWEEN :fechaInicial AND :fechaFinal AND d.conciliador.idConciliador=:idConciliador AND d.aceptada <> false AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
