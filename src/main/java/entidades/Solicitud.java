...
@NamedQueries({
	...
	@NamedQuery(name="Solicitud.findAudienciasFiltroConciliadorFecha", query="SELECT DISTINCT s FROM Agenda agd, Audiencia aud, Solicitud s, Designacion d, Conciliador c WHERE agd.fecha >= :fechaInicial AND agd.audiencia.idAudiencia = aud.idAudiencia AND aud.solicitud.idSolicitud = s.idSolicitud AND s.idSolicitud = d.solicitud.idSolicitud AND d.aceptada <> false AND d.conciliador.idConciliador = c.idConciliador AND c.identificacion=:identificacion AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
	//@NamedQuery(name="Solicitud.findAudienciasFiltroRadicadoFecha", query="SELECT DISTINCT s FROM Agenda agd, Audiencia aud, Solicitud s WHERE agd.fecha BETWEEN :fechaInicial AND :fechaFinal AND agd.audiencia.idAudiencia = aud.idAudiencia AND aud.solicitud.idSolicitud = s.idSolicitud AND s.nroRadicado = :nroRadicado AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
	@NamedQuery(name="Solicitud.findAudienciasFiltroRadicado", query="SELECT DISTINCT s FROM Agenda agd, Audiencia aud, Solicitud s WHERE agd.audiencia.idAudiencia = aud.idAudiencia AND aud.solicitud.idSolicitud = s.idSolicitud AND s.nroRadicado like :nroRadicado AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
	
	@NamedQuery(name="Solicitud.findAudienciasConciliador", query="SELECT DISTINCT s FROM Solicitud s, Designacion d, Audiencia aud, Agenda ag WHERE s.idSolicitud=d.solicitud.idSolicitud AND s.idSolicitud=aud.solicitud.idSolicitud AND aud.idAudiencia = ag.audiencia.idAudiencia AND ag.fecha >= :fechaInicial AND d.conciliador.idConciliador=:idConciliador AND d.aceptada <> false AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
	@NamedQuery(name="Solicitud.findAudienciasConciliadorFiltroParteFecha", query="SELECT DISTINCT s FROM Agenda agd, Audiencia aud, Solicitud s, Parte p, Designacion d WHERE agd.fecha >= :fechaInicial AND agd.audiencia.idAudiencia = aud.idAudiencia AND aud.solicitud.idSolicitud = s.idSolicitud AND s.idSolicitud = p.solicitud.idSolicitud AND p.identificacion = :identificacion AND p.tipoParte = :tipoParte AND s.idSolicitud = d.solicitud.idSolicitud AND d.aceptada <> false AND d.conciliador.idConciliador = :idConciliador  AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
	//@NamedQuery(name="Solicitud.findAudienciasConciliadorFiltroRadicadoFecha", query="SELECT DISTINCT s FROM Agenda agd, Audiencia aud, Solicitud s, Designacion d WHERE agd.fecha BETWEEN :fechaInicial AND :fechaFinal AND agd.audiencia.idAudiencia = aud.idAudiencia AND aud.solicitud.idSolicitud = s.idSolicitud AND s.nroRadicado = :nroRadicado AND s.idSolicitud = d.solicitud.idSolicitud AND d.aceptada <> false AND d.conciliador.idConciliador = :idConciliador AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
	@NamedQuery(name="Solicitud.findAudienciasConciliadorFiltroRadicado", query="SELECT DISTINCT s FROM Agenda agd, Audiencia aud, Solicitud s, Designacion d WHERE agd.audiencia.idAudiencia = aud.idAudiencia AND aud.solicitud.idSolicitud = s.idSolicitud AND s.nroRadicado like :nroRadicado AND s.idSolicitud = d.solicitud.idSolicitud AND d.aceptada <> false AND d.conciliador.idConciliador = :idConciliador AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
	...
	@NamedQuery(name="Solicitud.findSolicitudesFiltroRadicado", query="SELECT DISTINCT s FROM Solicitud s WHERE s.nroRadicado like :nroRadicado ORDER BY s.fecha")
